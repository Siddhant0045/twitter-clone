...
function Feed() {
  ...
  useEffect(() => {
    const fetchTweetsWithLikes = async () => {
      try {
        const res = await fetch("https://twitter-clone-kfoi.onrender.com/api/tweets");
        ...
        const tweetsWithLikes = await Promise.all(
          filteredTweets.map(async (tweet) => {
            const [countRes, likedRes] = await Promise.all([
              fetch(`https://twitter-clone-kfoi.onrender.com/api/likes/count/${tweet._id}`),
              fetch(
                `https://twitter-clone-kfoi.onrender.com/api/likes/isLiked?tweetId=${tweet._id}&userId=${currentUserId}`
              ),
            ]);
  ...
  const toggleLike = async (tweetId, isLiked) => {
    const url = "https://twitter-clone-kfoi.onrender.com/api/likes";
    const payload = {
      tweet: tweetId,
      user: currentUserId,
    };
